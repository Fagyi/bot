...
class CryptoBotApp:
    ...
    def _build_funds_tab(self):
        ...
        ttk.Button(preset_box, text="Trade → Cross", command=lambda: _preset("Trade", "Cross")).grid(row=1, column=0, sticky="ew", padx=(0, 6), pady=(0, 6))
        ttk.Button(preset_box, text="Cross → Trade", command=lambda: _preset("Cross", "Trade")).grid(row=1, column=1, sticky="ew", pady=(0, 6))
                    ...
                    if combined_sig in ('buy','sell') and (now - self._mb_last_cross_ts >= cooldown_s):
                        ...
                        if max_quote_for_trade <= 0.0:
                            ...
                            self._safe_log(
                                ...
                                f"lev={leverage} | mode={mode} dry={dry}\n"
                            )
                                        ...
                                        try:
                                            ...
                                            if code and str(code) != "200000":
                                                ...
                                                self._mb_hist_add_open(
                                                    ...
                                                    size=size_now, lev=leverage, fee=float(_fee_final),
                                                    pnl_est=pnl_est
                                                )
